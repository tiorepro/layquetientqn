...
(function () {
  ...
  function analyzeMonCungRelation(palace, chart) {
    ...
    // Tích hợp Đề xuất 3: Đo lường lực Bức Cung thông qua Nguyệt Lệnh
    const monthBranch = getBranchOf(chart, 'month');
    const monthHanh = MAP_CHI[monthBranch] || '';
    let mult = 1;
    let seasonSuffix = '';
    if (monthHanh) {
      const vt = getVangTuongState(gateHanh, monthHanh);
      if (vt.state === 'Vượng' || vt.state === 'Tướng') { mult = 1.5; seasonSuffix = ' (Hung hiểm TĂNG CAO do Môn đang vượng khí mùa này).'; }
      if (vt.state === 'Tử' || vt.state === 'Tù') { mult = 0.5; seasonSuffix = ' (Hung hiểm GIẢM NHẸ do Môn đang suy tù mùa này).'; }
    }

    if (khac(gateHanh, cungHanh)) {
      result.type = 'mon_buc_cung'; result.label = `${palace.batMon}(${gateHanh}) bức Cung(${cungHanh})`; result.score = -1 * mult;
      result.advice = 'Môn Bức Cung: Môn đang xung đột với nền tảng cung — hành động bề ngoài mạnh nhưng gốc rễ không vững.' + seasonSuffix;
    } else if (khac(cungHanh, gateHanh)) {
      result.type = 'cung_buc_mon'; result.label = `Cung(${cungHanh}) bức ${palace.batMon}(${gateHanh})`; result.score = -2 * mult;
      result.advice = 'Cung Bức Môn: Nền tảng đang kìm hãm hành động — sức mạnh của Môn bị triệt tiêu bởi vị trí đứng.' + seasonSuffix;
  ...
  function analyzePalaceFull(palace, roleName, chart) {
    ...
    if (palace.monBucChe) {
      if (palace.monBucChe.type === 'bức' || palace.monBucChe.type === 'chế') {
        let penalty = 1;
        if (typeof vtGate !== 'undefined' && vtGate) {
           if (vtGate.state === 'Vượng' || vtGate.state === 'Tướng') penalty = 1.5;
           if (vtGate.state === 'Tử' || vtGate.state === 'Tù') penalty = 0.5;
        }
        finalGateScore = finalGateScore > 0 ? Math.floor(finalGateScore / 2) : finalGateScore - penalty;
        score -= penalty;
      }
    }
    ...
    if (tk && !isVoid(chart, palace) && !hasRuMu(palace)) {
      html += UI.line(`  ↳ ✨ Tam Kỳ: ${tk.name}`, tk.desc, 'good');
        ...
        const _isVoid = isVoid(chart, palace);
    const _isHorse = isHorse(chart, palace);

    if (_isVoid && _isHorse) {
      html += UI.line('  ↳ ⭕ Không Vong + 🐎 Dịch Mã', 'Mã Động Phá Không: Sự việc ban đầu có vẻ ảo/đình trệ, nhưng Dịch Mã động sẽ sớm làm ứng nghiệm thành thực, không còn bị Không Vong khống chế hoàn toàn.', 'info');
      // Không trừ điểm Không Vong vì Mã đã phá Không
      ...
      if (opCung) {
        ...
        if (opPalace) {
           ...
           html += UI.note(`🔍 <strong>Chuyên gia (Đối Xung & Mã Động):</strong> Cung này ban đầu bị Không Vong, bản chất nằm ở cung Đối Xung (${palaceName(opCung, chart)}). Tại đó có <strong>${opG}</strong> và <strong>${opS}</strong>. Nhờ có Dịch Mã, sự việc sẽ nhanh chóng chuyển biến từ ảo thành thực!`, 'info');
        }
      }
    } else if (_isVoid) {
    ...
    if (_isHorse && !_isVoid) { html += UI.line('  ↳ 🐎 Dịch Mã', 'Di chuyển, thay đổi, bôn ba', 'info'); }
...
const yinDeities  = ['Thái Âm','Cửu Địa','Huyền Vũ'];
  ...
  TOPIC_ANALYZERS[26] = function(chart) {
    ...
    const addW = (type, source) => {
      ...
      wc[type].sources.push(source);
        ...
        html += UI.section('☁️ Quét Tín Hiệu Khí Tượng (Các Cung Trọng Điểm)');
    // ĐỀ XUẤT 5: Chỉ quét các cung quan trọng, tránh nhiễu
    const targetPalaces = new Set();
    const addPalaceToScan = (p) => { if (p) targetPalaces.add(p); };

    addPalaceToScan(getZhifuPalace(chart)); // Trực Phù (Xu thế chung)
    addPalaceToScan(getZhishiPalace(chart)); // Trực Sử (Môi trường thực tại)
    addPalaceToScan(findPalaceByStarName(chart, 'Thiên Bồng')); // Thần Mưa
    addPalaceToScan(findPalaceByStarName(chart, 'Thiên Anh')); // Thần Nắng
    addPalaceToScan(findPalaceByStarName(chart, 'Thiên Phụ')); // Thần Gió
    addPalaceToScan(findPalaceByHeavenlyStem(chart, getStemOf(chart, 'day'))); // Can Ngày
    addPalaceToScan(findPalaceByHeavenlyStem(chart, getStemOf(chart, 'hour'))); // Can Giờ

    for (const p of Array.from(targetPalaces)) {
  ...
  function runAnalysis(chartArg) {
    ...
    try {
      ...
      // ĐỀ XUẤT 2: NGĂN HIỆU ỨNG TRUNG HÒA - CAP ĐIỂM NẾU DỤNG THẦN CHÍNH XẤU
      const MAIN_DT_MAP = {
        2: 'Khai', 3: 'Khai', 5: 'Khai', 6: 'Khai', 9: 'Khai', 
        11: 'Sinh', 18: 'Lục Hợp', 21: 'Thiên Nhuế', 22: 'Khai', 23: 'Tân', 24: 'Can Giờ', 25: 'Can Giờ'
      };
      const mainDtName = MAIN_DT_MAP[Number(topicId)];
      if (mainDtName) {
        let pMain = null;
        if (mainDtName === 'Can Giờ') pMain = findPalaceByHeavenlyStem(chart, getStemOf(chart, 'hour'));
        else if (['Khai','Sinh','Hưu','Thương','Đỗ','Cảnh','Tử','Kinh'].includes(mainDtName)) pMain = findPalaceByGate(chart, mainDtName);
        else if (['Thiên Nhuế'].includes(mainDtName)) pMain = findPalaceByStarName(chart, mainDtName);
        else if (['Lục Hợp'].includes(mainDtName)) pMain = findPalaceByDeityName(chart, mainDtName);
        else if (mainDtName === 'Tân') pMain = findPalaceByHeavenlyStem(chart, 'Tân');

        if (pMain) {
           let mainScore = 0;
           if (isVoid(chart, pMain)) mainScore -= 3;
           if (hasRuMu(pMain)) mainScore -= 2;
           if (getEffectiveDeityName(pMain, chart).match(/(Bạch Hổ|Đằng Xà|Huyền Vũ)/)) mainScore -= 2;
           if (hasPattern(pMain, 'Lục Nghi Kích Hình') || hasPattern(pMain, 'kích hình')) mainScore -= 2;
           
           if (mainScore <= -5 && analysisObj.score > 0) {
              analysisObj.score = 0; // Khống chế tối đa 0 điểm
              analysisObj.html += `<div style="margin:10px 0;padding:8px;background:#fef2f2;border-left:4px solid #b91c1c;color:#991b1b;font-size:13px;">⚠️ <strong>CHUYÊN GIA HIỆU CHỈNH:</strong> Dụng thần chính (${mainDtName}) bị tổn thương quá nặng (Không Vong/Nhập Mộ/Hung Thần). Dù các yếu tố phụ có tốt đến đâu, đại cục vẫn không thể thành công. Điểm số được giới hạn tối đa ở mức Trung Hòa (0 điểm).</div>`;
           }
        }
      }
